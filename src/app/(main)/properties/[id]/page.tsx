...
export default function PropertyDetailPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success(res.data.data.saved ? '❤️ সংরক্ষিত' : 'সরানো হয়েছে')
  ...
  const { listing, ownerPhone } = data
  const images = listing.images || []
  const waLink = ownerPhone
    ? `https://wa.me/${ownerPhone.startsWith('0') ? '88' + ownerPhone : ownerPhone}?text=${encodeURIComponent(`PropertyBD থেকে "${listing.title}" সম্পর্কে জানতে চাই।`)}`
    : '#'
        ...
        <div className="detail-layout">
          ...
          <div>
            ...
            <div style={{ color: 'var(--text-secondary)', fontSize: '0.88rem', marginBottom: 20 }}>
              ...
              <span style={{ marginLeft: 12 }}>👁️ {listing.viewCount} বার দেখা</span>
            </div>
            ...
            <div style={{ display: 'flex', gap: 10, marginBottom: 24, flexWrap: 'wrap' }}>
              <a href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(typeof window !== 'undefined' ? window.location.href : '')}`}
                target="_blank" rel="noopener" style={{ padding: '8px 14px', borderRadius: 8, textDecoration: 'none', background: '#1877F2', color: 'white', fontWeight: 600, fontSize: '0.82rem' }}>
                📘 Facebook
              </a>
              <a href={`https://wa.me/?text=${encodeURIComponent(`PropertyBD-তে দেখুন: ${listing.title}`)}`}
                target="_blank" rel="noopener" style={{ padding: '8px 14px', borderRadius: 8, textDecoration: 'none', background: '#25D366', color: 'white', fontWeight: 600, fontSize: '0.82rem' }}>
                💬 WhatsApp শেয়ার
              </a>
            </div>
          </div>

          {/* Right: Contact card — সরাসরি number */}
          <div className="detail-contact-card" style={{ position: 'sticky', top: 72 }}>
            <div style={{ background: 'white', borderRadius: 16, border: '1px solid var(--border)', padding: 20, boxShadow: 'var(--shadow-sm)' }}>
              ...
              {/* Direct contact — no unlock needed */}
              {ownerPhone ? (
                <>
                  <div style={{ background: 'var(--green-light)', borderRadius: 10, padding: 14, textAlign: 'center', marginBottom: 10 }}>
                    ...
                    <a href={`tel:${ownerPhone}`} style={{ fontSize: '1.3rem', fontWeight: 800, color: 'var(--green-deep)', textDecoration: 'none' }}>
                      📞 {ownerPhone}
                    </a>
                  </div>
                  ...
                  <a href={waLink} target="_blank" rel="noopener" style={{
                    ...
                    background: '#25D366', color: 'white', fontWeight: 600, fontSize: '0.9rem', marginBottom: 8,
                  }}>
                    💬 WhatsApp করুন
                  </a>
                </>
              ) : (
                <div style={{ background: 'var(--surface-2)', borderRadius: 10, padding: 14, textAlign: 'center', marginBottom: 10, color: 'var(--text-muted)', fontSize: '0.88rem' }}>
                  নম্বর পাওয়া যায়নি
                </div>
              )}
        ...
        {similar.length > 0 && (
          <div style={{ marginTop: 48 }}>
            <div className="flex-between" style={{ marginBottom: 20 }}>
              <div style={{ fontSize: '1.2rem', fontWeight: 800 }}>একই এলাকার আরো বিজ্ঞাপন</div>
